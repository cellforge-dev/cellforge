...
const fallbackHomepage = "https://cellforge.dev";
const registryName = "@cellforge";
const allRegistryItemName = "all";

function toPublicItemName(slug: string): string {
  return slug
...
function toPublicComponentName(publicItemName: string): string {
...
const sharedSourceFiles: Array<{ absolutePath: string; targetPath: string; type: RegistryFile["type"] }> = [
  {
    ...
    targetPath: "components/ui/cellforge-core.tsx",
    type: "registry:lib"
  },
  {
    absolutePath: path.join(manualRoot, "dotmatrix-hooks.ts"),
    targetPath: "components/ui/cellforge-hooks.ts",
    type: "registry:lib"
  },
  {
    absolutePath: path.join(loadersRoot, "styles.css"),
    targetPath: "components/cellforge-loader.css",
    type: "registry:style"
  }
];
...
const importRewrites: ReadonlyArray<{ from: string; to: string }> = [
  { from: "../base/dot-matrix-base", to: "@/components/ui/cellforge-core" },
  { from: "../core/circle-mask", to: "@/components/ui/cellforge-core" },
  { from: "../core/cx", to: "@/components/ui/cellforge-core" },
  { from: "../core/grid-paths", to: "@/components/ui/cellforge-core" },
  { from: "../core/hydration-inline-style", to: "@/components/ui/cellforge-core" },
  { from: "../core/opacity-triplet", to: "@/components/ui/cellforge-core" },
  { from: "../core/dmx-dot-bloom", to: "@/components/ui/cellforge-core" },
  { from: "../core/color-presets", to: "@/components/ui/cellforge-core" },
  { from: "../core/path-wave-factory", to: "@/components/ui/cellforge-core" },
  { from: "../core/patterns", to: "@/components/ui/cellforge-core" },
  { from: "../types", to: "@/components/ui/cellforge-core" },
  { from: "../hooks/use-cycle-phase", to: "@/components/ui/cellforge-hooks" },
  { from: "../hooks/use-stepped-cycle", to: "@/components/ui/cellforge-hooks" },
  { from: "../hooks/use-prefers-reduced-motion", to: "@/components/ui/cellforge-hooks" },
  { from: "../core/phases", to: "@/components/ui/cellforge-hooks" }
];
...
function rewriteRegistrySharedCrossImports(source: string): string {
  return source
    .replaceAll('from "./dotmatrix-core"', 'from "@/components/ui/cellforge-core"')
    .replaceAll('from "./dotmatrix-hooks"', 'from "@/components/ui/cellforge-hooks"')
    .replaceAll('"@/components/dotmatrix-loader.css"', '"@/components/cellforge-loader.css"');
}

function brandRegistrySource(source: string): string {
  return source
...
async function build() {
  ...
  const sharedSources = await Promise.all(
    sharedSourceFiles.map(async (sharedFile) => {
      ...
      content = brandRegistrySource(content);
  ...
  const loaderRegistryItems = await Promise.all(
    loaderRegistry.map(async (loader) => {
      ...
      const publicItemName = toPublicItemName(loader.slug);
      const publicComponentName = toPublicComponentName(publicItemName);
      const componentSource = applyInstallDefaults(
        ...
        ).replaceAll(loader.componentName, publicComponentName)
          .replaceAll(loader.componentName.replace(/^Dotm/, "Cell"), publicComponentName),
        loader.slug
      );
      const brandedComponentSource = brandRegistrySource(componentSource);
      const componentPath = `components/ui/${publicItemName}.tsx`;
      files.push({
        path: componentPath,
        type: "registry:ui",
        content: brandedComponentSource
      });
      await writeRegistrySource(componentPath, brandedComponentSource);
      ...
      await writeFile(
        path.join(publicRegistryDir, `${publicItemName}.json`),
        JSON.stringify(item, null, 2) + "\n",
        "utf-8"
      );

      await writeFile(path.join(publicRegistryDir, `${publicItemName}.tsx`), brandedComponentSource, "utf-8");
  ...
  const allItem = {
    ...
    type: "registry:ui",
    title: "All CellForge Loaders",
    description: "Installs every CellForge loader and shared runtime files in one command.",
    dependencies: [],
    registryDependencies: [],
    meta: {
