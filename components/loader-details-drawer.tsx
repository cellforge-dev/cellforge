...
function toCellForgeComponentName(slug: string): string {
  return toCellForgeItemName(slug)
    .split("-")
...
export function LoaderDetailsDrawer({
  ...
  const scopedRegistryName = "@cellforge";
  const scopedItemName = selected ? `${scopedRegistryName}/${toCellForgeItemName(selected.slug)}` : "";
  const installCommand = selected ? shadcnRegistryAddCommand(packageManager, scopedItemName) : "";
  const selectedPublicItemName = selected ? toCellForgeItemName(selected.slug) : "";
  const selectedPublicComponentName = selected ? toCellForgeComponentName(selected.slug) : "";
  const demoUsageCode = selected
    ? `import { ${selectedPublicComponentName} } from "@/components/ui/${selectedPublicItemName}";

export function Example() {
  return <${selectedPublicComponentName} />;
}`
    : "";
  ...
  const propExampleCards = useMemo(() => {
    ...
    const C = toCellForgeComponentName(selected.slug);
    const from = toCellForgeItemName(selected.slug);
    const isSquareMatrix = from.startsWith("cell-square-");
    const bloomItem = {
